"""Seed DB with 2026-05-04 KPI data extracted from weekly_20260504.pptx."""
...
KPI_SEED = [
    {
        ...
        "highlights": [
            "Azure: cost until today $269",
            "AZRWHQDXLabPOCQ: $12",
            "AZRWHQDXLabPOCD: $31",
            "AWSWHQPROVISIONSITD: $105",
    ...
    },
    {
        ...
        "highlights": [
            ...
            "Nginx Ingress controller ç½®æ?ï¼Œé?è­?gateway-api + Traefik ä¸­ï?æ¸¬è©¦çµæ??‡å¯¦?½æ­¥é©Ÿæ”¾??cloud guidebookï¼Œå…­?ˆå??å??ç?ç©¶ï??è?å¹´å?ä¸Šç?ï¼ˆrke å»ºè­°ï¼šgateway-api å°šä?ç©©å? + rke è¦å?è£æ? custom ?ä?ä½¿ç”¨??v2.11.4 + Traefik dashboardï¼?,
            "CCoE mid-quarter report rehearsal & preparation",
        ],
        "lowlights": ["None"],
    },
    {
    ...
    },
    {
        ...
        "highlights": [
            ...
            "Slurm monitoring setting & SOPï¼Œæ¶è¨­æ–° UIï¼ˆAIF ?³è?ä¸­ï?ï¼Œä¸¦??Tony?Roger?TM?User èªªæ?ï¼Œç›®?é?è¨ˆè???ASPï¼Œå???slurmdbd æ­·å²è³‡æ?",
            "LLMOPS ??1. MinIO: API to create bucketï¼ˆç«¹?—ï?4/16ï¼?. BCM: Slurm å¸³è??NetApp ?›è??GPU Metrics 4/22ï¼?. Harbor: API account of harbor dev å·²å??ï?new harbor for AAW3.0 planned until end of Juneï¼?. K8S: service account YAML å»¶ç”¨?Ÿæœ¬?°ç«¯ GitLabï¼Œæ¸¬è©?ArgoCD apply service account",
        ],
        "lowlights": ["None"],
    },
    {
