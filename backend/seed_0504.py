...
KPI_SEED = [
    ...
    },
    {
        ...
        "highlights": [
            ...
            "Nginx Ingress controller 置換，驗證 gateway-api + Traefik 中，測試結果與實施步驟放至 cloud guidebook，六月底前完成研究，預計年底上線（rke 建議：gateway-api 尚不穩定 + rke 要包裝成 custom 提供使用者 v2.11.4 + Traefik dashboard）",
    ...
    },
    {
        ...
        "highlights": [
            ...
            "Slurm monitoring setting & SOP，架設新 UI（AIF 申請中），並與 Tony、Roger、TM、User 說明，目前預計融入 ASP，增加 slurmdbd 歷史資料",
            "LLMOPS — 1. MinIO: API to create bucket（竹北）4/16；2. BCM: Slurm 帳號、NetApp 掛載、GPU Metrics 4/22；3. Harbor: API account of harbor dev 已完成，new harbor for AAW3.0 planned until end of June；4. K8S: service account YAML 延用原本地端 GitLab，測試 ArgoCD apply service account",
        ],
        "lowlights": ["None"],
    },
    {
